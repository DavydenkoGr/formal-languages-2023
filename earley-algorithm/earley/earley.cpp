...
Situation::Situation(char nonterminal, const std::string& derivation,
                     uint32_t previous_position, uint32_t next_index)
    : rule{nonterminal, derivation}, previous_position(previous_position),
      next_index(next_index) {}

Situation::Situation(const Rule& rule, uint32_t previous_position,
                     uint32_t next_index)
...
Grammar::Grammar(char start, const std::vector<Rule>& rules)
    : start(start), rules(rules) {}

void AlgorithmEarley::Scan(const std::string& word, uint32_t position,
                           std::vector<std::map<char, std::set<Situation>>>& situations) const {
...
bool AlgorithmEarley::Predict(uint32_t position,
                              std::vector<std::map<char, std::set<Situation>>>& situations) const {
  bool changes = false;
...
bool AlgorithmEarley::Complete(uint32_t position,
                               std::vector<std::map<char, std::set<Situation>>>& situations) const {
  bool changes = false;
...
bool AlgorithmEarley::Earley(const std::string& word) const {
  ...
  for (uint32_t i = 0; i < word.length(); ++i) {
    ...
    while (changes) {
      changes = false;
      changes |= Predict(i + 1, situations);
...
void AlgorithmEarley::fit(const Grammar& new_grammar) {
...
bool AlgorithmEarley::predict(const std::string& word) const {
  return Earley(word);
}
...
void PredictRequests(const AlgorithmEarley& earley) {
